...
def evaluate(api_url: str = "http://localhost:8000/ask", test_file: str = "test/test_cases.json"):
    ...
    for case in tqdm(test_cases, desc="评估中"):
        try:
            # 使用 /ask 接口，payload 保持 {"question": "..."}
            resp = requests.post(api_url, json={"question": case["question"]}, timeout=15)
            if resp.status_code == 200:
                answer = resp.json().get("answer", resp.json().get("response", "无返回"))
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--api", default="http://localhost:8000/ask", help="FastAPI地址")
